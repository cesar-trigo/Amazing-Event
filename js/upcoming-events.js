...
imprimir(upcomingEvents, contenedorTarjet)
...
buscador.addEventListener("keyup", (e) => {
    ...
    let nombres = upcomingEvents.filter(value => value.name.toLowerCase().includes(inputUser.toLowerCase()));
...
contenedorcheckbox.addEventListener("change", event => {
    if (event.target.checked) {
        categorias = categorias.concat(upcomingEvents.filter(checkBox => checkBox.category.toLowerCase().includes(event.target.id.toLowerCase())))
        contenedorTarjet.innerHTML = ""
